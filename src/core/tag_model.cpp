...
void TagModel::import_images( const QFileInfoList& image_list )
{
...
bool TagModel::add_new_label( const QColor& color, const QString& label )
{
    if( label.isEmpty() )
    {
        return false;
...
void TagModel::remove_items( const QModelIndexList& index_list )
{
...
TagItem* TagModel::add_image_to_label( TagItem* label_item, const QFileInfo& image_file )
{
...
QString TagModel::get_fullpath( const QModelIndex& index ) const
{
    if( !index.isValid() )
    {
        return QString::null;
...
QString TagModel::get_label( const QModelIndex& index ) const
{
    if( !index.isValid() )
    {
        return QString::null;
...
QColor TagModel::get_color( const QModelIndex& index ) const
{
    if( !index.isValid() )
    {
        return QColor();
...
void TagModel::set_label( const QModelIndex &index, const QString& name )
{
...
void TagModel::set_color( const QModelIndex& index, const QColor& color )
{
...
TagItem* TagModel::get_tag_item( const QString& fullpath, const QString& label )
{
...
TagItem::Elements TagModel::get_element( const QString& fullpath, const QString& label )
{
...
QList<TagItem::Elements> TagModel::get_elements( const QModelIndexList& index_list ) const
{
...
QHash< QString, QList<TagItem::Elements> > TagModel::get_all_elements( const QModelIndexList& selection ) const
{
...
void TagModel::init_from_elements( const QHash< QString, QList<TagItem::Elements> >& elts, bool merge )
{
    if( !merge )
    {
        init();
...
QModelIndex TagModel::add_tag_to_label( const QString& fullpath, const QString& label, const QRect& tag )
{
    TagItem* image = get_tag_item( fullpath, label );
...
QModelIndex TagModel::remove_tag_from_label( const QString& fullpath, const QString& label, const QRect& tag )
{
    TagItem* image = get_tag_item( fullpath, label );
    if( !image )
    {
